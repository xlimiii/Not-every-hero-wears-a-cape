...
/*auto iterator = std::remove_if(std::begin(FLOWER), std::end(FLOWER), collision.isCollision(bee->getSprite(), FLOWER.at(i)));
			FLOWER.erase(iterator, std::end(FLOWER));*/

			//FLOWER.erase(FLOWER.begin(), FLOWER.end());

			//flower[i].setPos({ 422245 ,422246 }, FLOWER);
			/*for (std::vector<sf::Sprite>::iterator it = FLOWER.begin(); it != FLOWER.end(); )
			{
				FLOWER[i].setPosition
			}



			/*for (auto& flower : flowers)
				if (collision.isCollision(FLOWER.at(i), bee->getSprite())) break;*/
				//używamy break, bo kolizja następuje tylko jedna i nie musimy dalej iterować po całym kontenerze

				//sf::Vector2f beeCords = { bee->getPositionX(),bee->getPosition()};
				//auto iterator = remove_if(begin(FLOWER), end(FLOWER), [](sf::Sprite flower) {return flower.getGlobalBounds().contains(beeCords); }); //ta funkcja zwraca iterator na pierwszy zniszczony element
				////2 pierwsze wartości to zakres, po którym będziemy iterować, a 3. argumentem jest predykat w formie lambdy lub funkcji
				////Działa to tak, że bloczki które nie będą spełniać danego predyktatu zostaną przeniesione na początek wektora

				//FLOWER.erase(iterator, end(FLOWER));
				//Trzeba posortować usunięte bloczki, żeby były np. na końcu wektora